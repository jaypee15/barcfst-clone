(function ($) {
    ...
    let maskList = $.masksSort(pCodes, ['#'], /[0-9]|#/, "mask");
    let maskOpts = {
        inputmask: {
            definitions: {
                '#': {
    ...
    $('.js-mask-tel').each(function () {
        let $inputTel = $(this).find('input[type="text"]');
        inputTelId = $inputTel.attr('id');
        if (inputTelId) {
            $('#' + inputTelId).inputmask("remove");
            $('#' + inputTelId).inputmasks(maskOpts);
        }
    })

    $('.txtPhone').val('90').trigger('change');
